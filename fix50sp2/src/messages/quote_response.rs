...
pub struct QuoteResponse {
	...
	/// Conditionally required when countering a single instrument quote or "hit/lift" an IOI or Quote when applicable for the type
	/// of instrument.
	#[serde(flatten)]
	...
	#[serde(flatten)]
	pub leg_quot_grp: Option<super::super::leg_quot_grp::LegQuotGrp>,
	...
	/// The time when the quote will expire. Required for FI when the <a href="tag_694_QuoteRespType.html" target="bottom">QuoteRespType&nbsp;(694)</a> is either 1 (Hit/Lift) or 2 (Counter quote) to indicate to the Respondent when the counter offer is valid until.
	#[serde(skip_serializing_if = "Option::is_none")]
	#[serde(rename = "62")]
...
pub enum QuoteRespType {
	...
	#[serde(rename = "8")]
	TimedOut,
	/// Tied (Elaboration: Trade was done with another quote provider., quote provider's original quoted price was the same as the
	/// traded price.)
	#[serde(rename = "9")]
	Tied,
	/// Tied cover (Elaboration: Trade was done with another quote provider., quote provider's original quoted price was the best
	/// price not traded. There were other quote provider(s) at the same price)
	#[serde(rename = "10")]
	TiedCoverAtTheSamePrice,
	/// Accept
	#[serde(rename = "11")]
	Accept,
	/// Terminate contract
