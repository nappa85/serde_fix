...
use crate::entities::{EncodedText, LocalMktDate, MonthYear, RepeatingValues, UTCTimestamp};
...
pub struct EvntGrp {
    #[serde(rename = "864")]
    pub inner: RepeatingValues<EvntGrpItem>,
}

impl AsRef<Vec<EvntGrpItem>> for EvntGrp {
    fn as_ref(&self) -> &Vec<EvntGrpItem> {
        self.inner.as_ref()
    }
}

impl AsMut<Vec<EvntGrpItem>> for EvntGrp {
    fn as_mut(&mut self) -> &mut Vec<EvntGrpItem> {
        self.inner.as_mut()
    }
}
