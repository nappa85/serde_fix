...
where T: Serialize + DeserializeOwned + Default {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<RepeatingValues<T>, D::Error> {
        ...
        let parts = serde_fix::from_str::<Vec<(Cow<'_, str>, Cow<'_, str>)>>(parts).map_err(serde::de::Error::custom)?;
        ...
        for piece in pieces {
            ...
            inner.push(serde_fix::from_str(&s_piece).map_err(serde::de::Error::custom)?);
        }
...
where T: Serialize + DeserializeOwned + Default {
    fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
        ...
        for t in &self.0 {
            let mut s = serde_fix::to_string(t).map_err(serde::ser::Error::custom)?;
            if s.ends_with('\u{1}') {
